"""
Utility functions for parsing and formatting hotkey inputs.

Functions:
    parse_key(key_str): Convert string to pynput-compatible key.
    key_mouse_to_str(key): Convert key/mouse object to string.
"""
...
def key_mouse_to_str(key: Key | KeyCode | Button) -> str:
    ...
    Parameters:
        key (Key | KeyCode): The key or key code to convert.
    
    Returns:
        str: Readable representation such as "F6" or "A" or "right".
    """
    try:
...
def parse_key(key_str: str) -> Key | KeyCode:
    ...
    Parameters:
        key_str (str): Readable representation of a key (e.g., F6 or f6)
    
    Returns:
        Key | KeyCode: The classes of representing various keys that may not correspond to letters.
        if it has an error, it will log Unknown key
    """
...
def parse_button(button_str: str) -> Button:
    ...
    Parameters:
        button_str  (str): Readable button
    
    Returns:
        (Button | ValueError): The various keys. if it has an error, it will log ValueError
    """
    button_str = button_str.lower()
    try:
        if is_mouse_button(button_str=button_str):
